<template>
  ...
  <el-table ref="multipleTableRef"
            ...
            :default-sort="{prop: 'ereID', order: 'ascending'}">
    <el-table-column type="selection" />
    <el-table-column label="序号"
                     property="ereID"
                     sortable="custom" />
    <el-table-column property="examID"
                     label="考试编号" />
    <el-table-column label="考场序号"
                     property="examRoomID" />
    <el-table-column property="examName"
                     label="考试科目" />
    <el-table-column property="examLocation"
                     label="考试地点" />
    <el-table-column property="examRoomName"
                     label="考场名称" />
    <el-table-column property="examDate"
                     label="考试日期" />
    <el-table-column property="examTime"
                     label="考试时间段" />
    <el-table-column property="seatCount"
                     label="座位数" />
    <el-table-column property="proctors[0].teacherName"
                     label="监考员1" />
    <el-table-column property="proctors[1].teacherName"
                     label="监考员2" />
    <el-table-column label="操作"
      ...
      <template #default="{ row }">
        <el-button link
                   type="primary"
                   size="small"
                   @click="handleClickScan(row)">查看该场次考场的全部考生</el-button>
        <el-button link
                   type="primary"
                   size="small"
                   @click="handleClickEdit(row)">修改场次负责监考员</el-button>
        <el-button link
                   type="primary"
                   size="small"
                   @click="handleClickDelete(row)">清空该考场监考员</el-button>
      </template>
    </el-table-column>
  </el-table>
  ...
  <el-dialog v-model="dialogTableVisible"
    ...
    <el-table :data="studentTableData"
              ...
              element-loading-text="Loading..."
              :element-loading-spinner="svg"
              @sort-change="handleSortChangeStudent"
              :default-sort="{prop: 'seatID', order: 'ascending'}">
      <el-table-column label="学号"
                       property="studentID"></el-table-column>
      <el-table-column property="seatID"
                       label="座位号"
                       sortable="custom" />
    ...
    <div class="demo-pagination-block"
         style="margin-top: 10px;margin-left: 200px; margin-bottom: 20px;">
      <el-pagination v-model:current-page="dialogState.currentPage"
                     v-model:page-size="dialogState.pageSize"
                     :page-sizes="[10, 20, 30, 40]"
                     :small="dialogState.small"
                     :disabled="dialogState.disabled"
                     :background="dialogState.background"
                     layout="total, sizes, prev, pager, next, jumper"
...
export default {
  setup () {
    ...
    const getTableDataStuent = () => {
      loadingStudent.value = true
      axios.get("http://localhost:8080/student/selectStudentOfExamRoom", {
        params: {
