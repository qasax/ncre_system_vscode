<template>
  ...
  export default {
    setup() {
      ...
      let proctors =ref([]) 
      onMounted(()=>{
        axios.get("http://localhost:8080/selectIt",{params:{
      ...
      const rules = reactive(
        {
          'proctors[0].proctorID': [{ required: true, message: '请选择监考员', trigger: 'change' }],
        ...
        formEl.validate((valid) => {
          if (valid) {
            ...
            ElMessageBox.confirm(
              ...
              .then(() => {
                console.log(ruleForm)
                  axios.get('http://localhost:8080/ereProctors/update', {
                    params:{
